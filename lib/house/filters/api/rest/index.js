...
(exports = module.exports = function(house, options){
                ...
                for(var i in data) {
                    ...
                    if(it.hasOwnProperty('itunes')) {
                        ...
                        if(itunes.hasOwnProperty('image')) {
                            //str = str + '<itunes:image href="'+house.config.site.url+'api/files/'+he.encode(itunes.image.filename)+'"/>';
                            str = str + '<itunes:image href="'+he.encode(itunes.image)+'"/>';
                        } else if(house.config.site.logo) {
                            str = str + '<itunes:image href="'+he.encode(house.config.site.url+house.config.site.logo)+'"/>';
                            //str = str + '<image><title>'+he.encode(house.config.site.title)+'</title><url>'+he.encode(house.config.site.url+house.config.site.logo)+'</url><link>'+he.encode(house.config.site.url)+'</link></image>';
                        }
                    }
                    if(it.hasOwnProperty('media')) {
                        ...
                        //str = str + '<enclosure url="'+house.config.site.url+'api/files/'+media.filename+'" length="'+media.length+'" type="'+he.encode(media.contentType)+'" />';
                        str = str + '<enclosure url="'+media.url+'" length="'+media.length+'" type="'+he.encode(media.contentType)+'" />';
                    }
                    ...
                    items += '<item>\
    <title>'+he.encode(it.title)+'</title>\
    '+linkStr+'\
    ...
    <pubDate>'+it.pubDate+'</pubDate>'+str+'\
                ...
                if(house.config.site.title) {
                    ...
                    if(mediaFormat) {
                        ...
                        if(house.config.itunes && house.config.itunes.hasOwnProperty(mediaFormat.toLowerCase()) && house.config.itunes[mediaFormat.toLowerCase()].newFeedUrl) {
                            desc = desc + '<itunes:new-feed-url>'+he.encode(house.config.itunes[mediaFormat.toLowerCase()].newFeedUrl)+'</itunes:new-feed-url>';
                        }
                    } else {
                        if(house.config.itunes && house.config.itunes.newFeedUrl) {
                            desc = desc + '<itunes:new-feed-url>'+he.encode(house.config.itunes.newFeedUrl)+'</itunes:new-feed-url>';
                        }
                    }
                    title = '<title>'+he.encode(house.config.site.title+t)+'</title>';
                }
                ...
                if(house.config.site.url) {
                    title = title + '<atom:link href="'+he.encode(house.config.site.url+req.url.substr(1))+'" rel="self" type="application/rss+xml"/>';
                    link = '<link>'+he.encode(house.config.site.url)+'</link>';
                    if(house.config.site.logo) {
                        link = link + '<itunes:image href="'+he.encode(house.config.site.url+house.config.site.logo)+'"/>';
                        link = link + '<image><title>'+he.encode(house.config.site.title)+'</title><url>'+he.encode(house.config.site.url+house.config.site.logo)+'</url><link>'+he.encode(house.config.site.url)+'</link></image>';
                    }
                }
                if(house.config.site.desc) {
                    desc = desc + '<description>'+he.encode(house.config.site.desc)+'</description>';
                    desc = desc + '<itunes:summary>'+he.encode(house.config.site.desc)+'</itunes:summary>';
                }
                ...
                if(house.config.site.owner) {
                    author = '<itunes:author>'+he.encode(house.config.site.owner.name)+'</itunes:author>';
                    author = author + '<itunes:owner><itunes:name>'+he.encode(house.config.site.owner.name)+'</itunes:name><itunes:email>'+he.encode(house.config.site.owner.email)+'</itunes:email></itunes:owner>';
                    author = author + '<copyright>&#x2117; &amp; &#xA9; '+he.encode(house.config.site.owner.name)+'</copyright>';
                }
