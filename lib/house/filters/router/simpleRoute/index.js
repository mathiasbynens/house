...
(exports = module.exports = function(house){
    ...
    var handleReq = function(req, res, next) {
        ...
        house.log.debug('route url: '+path);
        // 
        ...
        for(var r in router) {
            for(var routePath in router[r]) {
                if(path.indexOf(routePath) === 1) {
                    ...
                    var matchedUrl = path.substr(routePath.length+1);
                    if(matchedUrl == '') {
                        //req.url = '/';
                        house.log.debug('redirect to +/');
                        house.utils.response.redirect(res, path+'/');
                    } else {
                        req.urlRouted = matchedUrl;
