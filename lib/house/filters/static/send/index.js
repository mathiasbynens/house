...
exports = module.exports = function(house, options) {
    ...
    var handleReq = function(req, res, next) {
        ...
        var getLessFromString = function(lessString, filename, paths, callback) {
            var lessOpts = {}
        ...
        var getCssFromLessPath = function(lessPath, callback) {
            ...
            var lessString = '';
            var paths = [];
            var pushPath = function(str) {
                var parentFolder = str.substr(0, str.indexOf(lessStr));
                var parentFolderI = parentFolder.lastIndexOf('/');
                paths.push(parentFolder.substr(0, parentFolderI));
            }
            ...
            getLessFromString(lessString, filename, paths, callback);
        }
        ...
        if(req.url.indexOf('require.js?/') !== -1) {
            ...
            if(lessPath.indexOf('?') !== -1) {
                lessPath = lessPath.substr(0, lessPath.indexOf('?'));
            }
            ...
            if(options.cachedHtml && !req.cacheTry) {
                publicFolder = options.cachedHtml;
                if(options.otherwise && path === '/' || path === '') {
                    path = path + options.otherwise;
                    req.cacheTry = true;
                } else {
                    if(req.cacheTryOne) {
                        path = path + '.html';
            ...
            send(req, path)
                ...
                .on('error', function(err) {
                    ...
                    req.url = req.urlOrig;
                    if(req.cacheTryOne && !req.cacheTry) {
                        handleReq(req, res, next);
                    } else if(req.cacheTry && !req.cacheTryOff) {
                        // var path = url.parse(req.url).pathname;
                        // var publicFolder = req.cachedHtml || options.publicFolder;
                        req.cacheTryOff = true;
                        handleReq(req, res, next);
                    } else if(options.otherwise && !req.otherwiseTry) {
                        if(req.cachedHtml) {
