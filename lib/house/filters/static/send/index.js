    ...
    var handleReq = function(req, res, next) {
        ...
        var getCssFromLessPath = function(lessPath, filename, callback) {
            ...
            fs.readFile(lessPath, function (err, data) {
              if (err) {
                  house.log.debug('error with fs.readFile: '+lessPath);
                  if(callback) {
                      callback(err);
                  }
              } else if(data) {
                ...
                less.render(dataStr, lessOpts, function (e, css) {
                    // TODO map
                    if(house.config.env !== 'dev') {
                ...
                if(!house.cssCacheWatch.hasOwnProperty(lessPath)) {
                    ...
                    try {
                        fs.watch(lessPath, function(event, watchFilename) {
                            // console.log('event is: ' + event+' for '+lessPath);
                            // if (watchFilename) {
                                // console.log('watchFilename provided: ' + watchFilename);
                            // } else {
                                // console.log('watchFilename not provided');
                            // }
