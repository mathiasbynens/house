...
(exports = module.exports = function(config){
    ...
    var ConfigEvents = function() {
        if (!(this instanceof ConfigEvents)) {
            return new ConfigEvents();  
        } 
        events.EventEmitter.call(this);
    }
    util.inherits(ConfigEvents, events.EventEmitter);
    house.config = new ConfigEvents();
    
    house.setConfig = function(config) {
        // this.config = config;
        for(var c in config) {
            house.config[c] = config[c];
        }
