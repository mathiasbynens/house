    ...
    house.start = function(cb) {
        ...
        if(house.config.ssl) {
            ...
            house.io.sockets.on('connection', function (socket) {
                house.log.debug('io secure socket connection');
            });
            if(!house.config.requireSsl) {
                ...
                house.ioi.sockets.on('connection', function (socket) {
                    house.log.debug('io insecure socket connection');
                });
        ...
        var webIp = house.config.webIp || '127.0.0.1';
        if(house.config.ssl) {
            ...
            // Listen on the port specified via config
            house.httpServer.listen(house.config.webPort, webIp, function(){
                var serverAddress = house.httpsServer.address();
                console.log('Server running at http://'+serverAddress.address+':'+serverAddress.port+'/');
                if(cb) cb();
            });
        }
