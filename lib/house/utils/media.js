(function(){
    ...
    media.fs.getFileMime = function(fullPath, callback) {
        var getMimeViaExif = function(callback) {
            ...
            exif.stderr.on("data", function(data) {
                console.log("stderr exiftool: " + data);
                console.dir(data);
                if(!calledback) {
                    //calledback = true;
        ...
        media.gridfs.getUniqueFilename(db, filesCol, file_name, function(unique_file_name) {
            ...
            uploadFileGridStore.writeFile(filePath, function(err, gridFile){
                if(err) {
                    ...
                    //house.log.debug('uploadFileGridStore.writeFile success');
                    //house.log.debug(gridFile);
                
                    callback(null, gridFile);
                }
                /*
                var uploadedGridFile = new mongo.GridStore(db, unique_file_name, "r", {'root': filesCol});
                uploadedGridFile.open(function(err, gridStore) {
                    if(err) {
                        house.log.err(err);
                        callback(err);
                    } else {
    ...
    media.gridfs.importUrl = function(db, filesCol, url, metadata, callback) {
        ...
        media.url.importFileToTmp(url, function(err, fullPath, responseHeaders) {
            //console.log('media.url.importFileToTmp fullPath: '+fullPath);
            if(err) {
                house.log.err(err);
            } else if(fullPath) {
                media.fs.getFileMime(fullPath, function(err, mimeType){
                    ...
                    if(err) {
                        house.log.err('media.fs.getFileMime err');
                        house.log.err(err);
                    } else if(mimeType) {
                        house.log.debug('mime type: '+mimeType);
    ...
    media.url.importFileToTmp = function(url, callback) {
        ...
        var headersPath = "/tmp/" + encodeURIComponent(tmpPath)+"header.txt";
        ...
        // -i to return headers as well
        var curlResponseHeaders = {};
        var curlData = '';
        var head = false;
        var breakStr = '\r\n\r\n'; // break string between headers and response data
        
        var curl = spawn("curl", ["--dump-header", headersPath, "--compressed", "--user-agent", userAgent, "-L", url ]);
        //var curlHeadersOnly = spawn("curl", ["-I", "--compressed", "--user-agent", userAgent, "-L", url ]); // -i outputs headers // --dump-headers filepath.txt
        
        curl.stdout.on("data", function(data) {
            // TODO CLEANUP
            // I don't like this but it's working... needed to slice the utf8 portion of the curl output (headers)
            // Could break if the headers dont fit into the first data buffer
            // Could replace with 2 curl calls? 1 for headers, 1 for data
            
            if (data) {
                /*
                curlData += data.toString('utf8');
                
                if(!head) {
                    var breakIndex = curlData.lastIndexOf(breakStr);
                    if(breakIndex !== -1) {
                        var headData = curlData.substr(0, breakIndex+breakStr.length);
                        fileStream.write(data.slice(Buffer.byteLength(headData, 'utf8')));
                        head = true;
                    } else {
                    }
                } else {
        ...
        curl.on("close", function(code) {
            ...
            /*
            var breakIndex = curlData.lastIndexOf(breakStr);
            var htmlData = curlData.substr(breakIndex+breakStr.length);
            */
            // console.log(htmlData);
            //var fileBuffer = new Buffer(htmlData, 'binary');
            //fileStream.write(fileBuffer, 'binary'); // .slice(0, 3);
        });
    };
})();