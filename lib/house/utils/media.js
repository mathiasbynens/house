(function() {
    ...
    media.gridfs.importUrl = function(db, filesCol, urlStr, metadata, callback) {
        var urlFilenameSansPath = urlStr;
        var parsedUrl = url.parse(urlStr);
        urlFilenameSansPath = parsedUrl.pathname;
        var lastSlashI = urlFilenameSansPath.lastIndexOf('/');
        if(lastSlashI !== -1) {
            urlFilenameSansPath = urlFilenameSansPath.substr(lastSlashI+1);
        }
        urlFilenameSansPath = parsedUrl.hostname + '/' + urlFilenameSansPath;
        var urlFilename = "/url/" + decodeURIComponent(urlFilenameSansPath).replace(/[:\/]/g, ".");
        media.url.importFileToTmp(urlStr, function(err, fullPath, responseHeaders) {
            ...
            if(err) {
                ...
                var importMimeFileToGrid = function(mimeType) {
                    ...
                    media.exif.getFromPath(fullPath, function(exif) {
                        ...
                        media.gridfs.importFile(db, filesCol, urlFilename, fullPath, mimeType, metadata, function(err, gridfile) {
