...
(exports = module.exports = function(house, options) {
    ...
    var authUrl = house.config.site.authUrl || (siteUrl + 'api/auth');
    ...
    var sendResetLinkTxtTo = function(txt, to, callback) {
        ...
        var fromUser = house.config.email.user;
        var sub = "Password Recovery";
        if(house.config.site.title) {
            sub = house.config.site.title + ' ' + sub;
        }
        var sendEmailOpts = {
            text: txt,
            from: '<' + fromUser + '>',
        ...
        if(house.config.site.owner && house.config.email.user == house.config.site.owner.email) {
            sendEmailOpts.from = house.config.site.owner.name + ' <' + house.config.site.owner.email + '>';
        }

        house.log.debug('send reset link email');
    ...
    var handleReq = function(req, res, next) {
        ...
        var secureReq = function() {
            if(house.config.ssl && !req.isSecure) {
                var reqPath = req.url; // '/api/auth'
                house.log.debug('insecure connection to auth endpoint, redirect to /api/auth'+path);
            ...
            if(path === '' || path === '/') {
                ...
                ds.find(col, {
                    ...
                    if(err) {
                        ...
                        res.writeHead(302, {
                            'Location': house.config.site.url
                        });
                        res.end();
                        ...
                        house.log.debug("Reset hash not found, or has already been used.");
                        res.setHeaderAccessControl();
                        res.writeHead(302, {
                            'Location': house.config.site.url
                        });
                        ...
                        function(error, oauth_token, oauth_token_secret, additionalParameters) {
                            if(error) {
                                ...
                                var redirectTwitterUrl = function(twitterUser) {
                                    ...
                                    /* TODO needs testing and task queuing / throttling
                ...
                if(req.fields.hasOwnProperty('email') && req.fields.hasOwnProperty('resetPass') && req.fields.resetPass) {
                    ...
                    ds.update(col, {
                        ...
                        if(err) {
                            ...
                            house.log.debug(docs);
                            var siteLoginUrl = authUrl;
                            if(siteLoginUrl.indexOf('https') !== 0) {
                                siteLoginUrl = siteLoginUrl.replace('http', 'https')
                            }
                            var resetUrl = siteLoginUrl + '/reset?pass=' + tmpPass + '&redirect=' + encodeURIComponent(ref);
                            var txt = "To reset your account password, click the following link: " + resetUrl + " .";
