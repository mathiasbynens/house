...
(exports = module.exports = function(house, options){
    ...
    if(house.config.stripe && house.config.stripe) {
        var testMode = true;
        if(house.config.stripe.hasOwnProperty('isLive')) {
            testMode = (!house.config.stripe.isLive);
        }
        ...
        if(paymentProcessor.stripe.privateKey) {
            var stripe = require("stripe")(paymentProcessor.stripe.privateKey);
        } else {
            return new Error('Stripe module requires privateKey');
        }
