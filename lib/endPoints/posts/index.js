...
(exports = module.exports = function(house, options){
    ...
    var ensureIndexes = function() {
        ...
        collection.ensureIndex({"at": -1, "slug": 1}, function(){
            house.log.debug('index ensured for posts at slug');
        });
    };
    ...
    var warmUpCache = false;
    if(house.config.hasOwnProperty('warmCache')) {
        warmUpCache = house.config.warmCache || 30000;
    }
    ...
    var handleReq = function(req, res, next) {
        ...
        var countQuery = function(query) {
            ...
            if(req.session.data.groups && req.session.data.groups.indexOf('admin') !== -1) {
                if(query.hasOwnProperty('groups') && query.groups === '') {
                    query["groups"] = null;
                }
        ...
        if(path.length > 1 && (path.indexOf(tagsPrefix) !== -1)) {
            ...
            house.log.debug('posts endpoint - subPath: '+subPath);
        }
        
        if(req.method == 'GET') {
            var query = {};
            if(subPath && subPath === tagsPrefix) {
                house.log.debug('posts endpoint GET tags');
                query._id = docId;
            ...
            house.log.debug('posts endpoint POST');
            if(subPath && subPath === tagsPrefix) {
                    ...
                    ds.find(tagsCol, tagQuery, function(err, data){
                        if(err) {
                            ...
                            if(data.length > 0) {
                                ...
                                ds.update(tagsCol, {_id: doc.id}, {$inc:{"postsCount": 1}}, function(err, data){
                                    
                                });
                                
                                callback(null, doc);
                            } else {
                                newObject.postsCount = 1;
                ...
                findOrInsertTag(newObject, function(err, tag){
                    if(err) {
                        ...
                        if(docId) {
                            query._id = docId;
                            ds.update(col, query, {$push: {"tags": tag}}, function(err, data){
                                res.data(tag);
                            });
                        }
                    }
                        ...
                        ds.find(col, query, function(err, docs) {
                            if (err) {
                                ...
                                if(true) {
                                    ...
                                    if(updatedDoc.hasOwnProperty('groups') && updatedDoc.groups && updatedDoc.groups.indexOf('public') !== -1) {
            ...
            if(docId) {
                ...
                if(putDoc.hasOwnProperty('$unset') && putDoc["$unset"].hasOwnProperty('owner')) {
                    // take ownership
                    if(req.session.data.hasOwnProperty('groups') && req.session.data.groups.indexOf('admin') !== -1) {
                        if(!putDoc.hasOwnProperty('$set')) {
                            putDoc["$set"] = {};
                        }
                        putDoc["$set"]["owner"] = {
                            id: req.session.data.user,
            ...
            if(docId && subPath && subPath.indexOf(tagsPrefix) === 0) {
                var tag_id = subPath.substr(tagsPrefix.length + 1);
                try {
                    tag_id = new ObjectID(tag_id);
                    house.log.debug('delete tag_id:');
                    console.log(tag_id);
                    query["_id"] = docId;
                    ds.update(col, query, {$pull: {"tags": {id: tag_id}}}, function(err, data){
                        ds.update(tagsCol, {_id: tag_id}, {$inc:{"postsCount": -1}}, function(err, data){
                        });
                        res.data(data);
                    });
                    return;
                    ...
                    house.log.err(e);
                    house.log.debug('delete OLD tag_id: '+tag_id);
                    query["_id"] = docId;
                    var updateDoc = {$pull: {"tags": decodeURIComponent(tag_id)}};
