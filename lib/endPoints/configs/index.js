...
(exports = module.exports = function(house, options){
    ...
    //     ensureIndexes();
    // });
    var cacheDir = process.cwd() + "/cache" + "/configs/";
    var configCacheJsPath = cacheDir + 'config.js';
    var writeClientConfig = function(jsObj, callback){
        var str = '(function() {\n\
...
\n\
    if (define) {\n\
        ...
        fs.writeFile(configCacheJsPath, str, function(err){
            if(err) {
                house.log.err(err);
                //res.data({});
            } else{
    ...
    var parseDbConfigDocIntoHouse = function(doc) {
        ...
        if(doc.client) {
            self.clientConfig[doc.key] = doc.value;
        } else {
    ...
    var refreshConfigs = function() {
        ds.find(col, {}, function(err, data){
            if(err) {
                ...
                writeClientConfig(self.clientConfig);
            } else {
