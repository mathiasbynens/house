...
(exports = module.exports = function(house, options){
    ...
    var incUserCheckins = function(userId, field, b) {
        incUserField(userId, 'checkinsCount', b);
    }
    ...
    var handleReq = function(req, res, next) {
        ...
        var removeDocFromFeed = function(doc) {
            if(doc.feed && doc.feed.id) {
                ...
                var feedQuery = {"ref": {"col": "checkins", "id": doc.id}};
        ...
        var procMapImage = function(doc, callback) {
            console.log('procMapImage');
            console.log(doc);
            var mapRequestUrl = 'https://maps.google.com/maps/api/staticmap?center='+doc.loc[0]+','+doc.loc[1]+'&zoom=15&size=640x640&maptype=terrain&sensor=false';
            
            var urlsEndPoint = house.api.getEndPointByName("urls");
            urlsEndPoint({session: req.session, method: 'POST', url: '', fields: {url: mapRequestUrl}}, {end:function(){}, data:function(newData){
                if(_.isArray(newData)) {
                    newData = _.first(newData);
                }
                if(newData.image) {
                    ds.update(col, {_id:doc.id}, {$set:{proc:1, mapImage: newData.image}}, function(err, data){
                        if(err) {
                            house.log.err(err);
                            res.end('error');
                        } else {
        ...
        if(req.method == 'GET') {
            ...
            if(path == '') {
                ...
                ds.insert(col, newDoc, function(err, data){
                    if(err) {
                        ...
                        incUserCheckins(req.session.data.user, 1);
                        if(data.hasOwnProperty('loc')) {
                            procMapImage(data, function(err, mapUrlData){
                                //data.mapImage = mapUrlData.image;
            ...
            house.log.debug('delete checkin');
            if(!req.session.data.user) {
                res.writeHead(403);
                res.end('{}');
                return;
