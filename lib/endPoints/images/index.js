    ...
    var handleReq = function(req, res, next) {
        ...
        var findQuery = function(query) {
            ...
            if (req.session.data.groups && req.session.data.groups.indexOf('admin') !== -1) {} else if (req.session.data.user) {
                ...
                if (req.session.data.groups) {
                    var inArr = _.clone(req.session.data.groups);
                    inArr.push('public');
                    query["$or"].push({
                        "groups": {
                            $in: inArr
        ...
        var removeDocFromFeed = function(doc) {
            if (doc.feed && doc.feed.id) {
                house.log.debug('remove image from feed');
                feedEndPoint({
                    session: req.session,
                    method: 'DELETE',
                    url: '/' + doc.feed.id,
        ...
        var removeDocRefs = function(doc, callback) {
            if (doc.hasOwnProperty('checkin')) {
                house.log.debug('remove checkin from image');
                // house.log.debug(checkinsEndPoint);
                checkinsEndPoint({
                    session: req.session,
                    method: 'DELETE',
                    url: '/' + doc.checkin.id,
        ...
        var deleteSizesFiles = function(doc, callback) {
            var fileIds = [];
            var deleteFileId = function(callback) {
                var fileId = fileIds.pop();
                // console.log('remove file from image');
                filesEndPoint({
                    session: req.session,
                    method: 'DELETE',
                    url: '/' + fileId,
                    ...
                    data: function(data) {
                        // console.log('removed file from image');
                        if(fileIds.length > 0) {
                            deleteFileId(callback);
                        } else {
                            callback();
                        }
                    },
            ...
            if(doc.sizes) {
                for(var s in doc.sizes) {
                    var size = doc.sizes[s];
                    if(size.id) {
                        fileIds.push(size.id);
                    }
                }
                deleteFileId(function(){
                    callback();
                });
            } else {
                callback();
                            ...
                            var withImageDimensions = function(origW, origH) {
                                ...
                                var procSizeFull = function(fullPath, callback) {
                                    ...
                                    if (true || fullSize < origW || fullPath.substr(-7) == '.gif[0]') {
                                        ...
                                        house.utils.media.imagemagick.convertToGrid(convertFullToFull, tmpFullPath, ds.db, filesRoot, "images/full/" + newFileNameBase, mimeType, newFileMeta, function(err, gridStore, tmpFullPath) {
                                            if(err) {
                                                house.log.err(err);
                                            } else if(gridStore) {
                                ...
                                var procSizeMedium = function(fullPath, callback) {
                                    if (mediumSize < origW) {
                                        ...
                                        house.utils.media.imagemagick.convertToGrid(convertFullToFull, tmpMediumPath, ds.db, filesRoot, "images/medium/" + newFileNameBase, mimeType, newFileMeta, function(err, gridStore, tmpMediumPath) {
                                            if(err) {
                                                house.log.err(err);
                                            } else if(gridStore) {
                                                var data = house.utils.media.gridfs.getDocFromGridStore(gridStore);
                                ...
                                var procSizeSmall = function(fullPath, callback) {
                                    if (smallSize < origW) {
                                        ...
                                        house.utils.media.imagemagick.convertToGrid(convertFullToFull, tmpSmallPath, ds.db, filesRoot, "images/small/" + newFileNameBase, mimeType, newFileMeta, function(err, gridStore, tmpSmallPath) {
                                            if(err) {
                                                house.log.err(err);
                                            } else if(gridStore) {
                                                var data = house.utils.media.gridfs.getDocFromGridStore(gridStore);
                                ...
                                var procSizeSquare = function(fullPath, callback) {
                                    if(origW !== origH) {
                                        ...
                                        house.utils.media.imagemagick.convertToGrid(convertFullToSquare, tmpSquarePath, ds.db, filesRoot, "images/square/" + newFileNameBase, mimeType, newFileMeta, function(err, gridStore, tmpSquarePath) {
                                            if(err) {
                                                house.log.err(err);
                                            } else if(gridStore) {
                                                var data = house.utils.media.gridfs.getDocFromGridStore(gridStore);
                                ...
                                var procSizeThumb = function(tmpSquarePath, callback) {
                                    ...
                                    if(thumbSize < origW && thumbSize < origH) {
                                        ...
                                        house.utils.media.imagemagick.convertToGrid(convertSquareToThumb, tmpThumbPath, ds.db, filesRoot, "images/thumb/" + newFileNameBase, mimeType, newFileMeta, function(err, gridStore, tmpThumbPath) {
                                            if(err) {
                                                house.log.err(err);
                                            } else if(gridStore) {
                                                var data = house.utils.media.gridfs.getDocFromGridStore(gridStore);
                ...
                ds.update(col, query, putDoc, function(err, data) {
                    if (err) {
                        ...
                        ds.find(col, query, function(err, data) {
                            ...
                            } else if (updatedDoc.hasOwnProperty('feed') && updatedDoc.feed !== 0) {
