...
(exports = module.exports = function(house, options){
    ...
    house.io.emitToRoomDocAndOwnerId = function(col, verb, doc, ownerId, secureOnly) {
        house.log.debug('house.io.emitToRoomDocAndOwnerId - '+col+' '+verb);
        house.log.debug(doc);
        house.log.debug(ownerId);
        house.log.debug(secureOnly);
        var colVerb = verb+col.charAt(0).toUpperCase() + col.substr(1);
        if(_.isArray(doc)) {
            _.each(doc, function(doc) {
                house.io.emitToRoomDocAndOwnerId(col, verb, doc, ownerId, secureOnly);
            });
            return;
        }
        if(verb == 'deleted') {
            ...
            for(var id in ioRoomManager.handshaken) {
                ...
                if(handshake.session.user && ownerId) {
                    if(ownerId.toString() == handshake.session.user.toString()) {
