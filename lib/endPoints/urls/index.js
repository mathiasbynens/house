...
(exports = module.exports = function(house, options) {
    ...
    var cacheShareUrlHtml = function(doc, callback) {
        if(house.config.env === 'dev') {
            house.log.debug('no html caching on dev');
            if(callback) {
                callback();
            }
            return;
    ...
    var handleReq = function(req, res, next) {
        ...
        var findMyUrlDoc = function(urlStr, callback) {
            var query = {
                url: urlStr
            };
            if(!req.session.data.user) {
                house.log.debug('no urls for non-user');
            }
        ...
        var emitToRoomIn = function(col, verb, doc) {
            if(!house.io || !house.io.rooms) {
                ...
                if(verb === 'deleted') {
                    house.ioi.emitDocToRoomOwnerGroup(col, verb, doc);
                } else {
        ...
        var removeFilesViaDoc = function(doc, callback) {
            ...
            if(doc.file) {
                removeFile(doc.file.id, function(){
                    console.log('removed File------------');
                    if(doc.image) {
                        removeImage(doc.image.id, function(){
        ...
        var cloneGridStoreFile = function(gs) {
            var fileObj = house.utils.media.gridfs.getDocFromGridStore(gs);
            if(gs.id) {
                fileObj.id = gs.id;
            }
            if(gs.length) {
        ...
        var processUrls = function(urls, callback, copyOldArticles) {
            ...
            if(urlDoc.proc === 1) {
                ...
                if(urlDoc.hasOwnProperty('lastUpdate')) {
                    var updateRate = 1000 * 60 * 10;
                    var now = new Date();
                    if(urlDoc.lastUpdate.getTime() + updateRate > now.getTime()) {
                        house.log.debug('url is existing proccessed with lastUpdate+updaterate < now');
                        doSkipProc = true;
                    }
                } else if(urlDoc.hasOwnProperty('nextUpdate')) {
                    if(urlDoc.nextUpdate > new Date()) {
                        house.log.debug('url is existing proccessed with nextUpdate > now');
                        doSkipProc = true;
                    }
                }
                ...
                if(urlDoc.hasOwnProperty('file')) {
                    if(urlDoc.file.contentType.indexOf('image') === 0) {
                        house.log.debug('url is existing proccessed image');
                        doSkipProc = true;
                    }
                }
                ...
                if(err) {
                    ...
                    if(gridfile.contentType.indexOf('image') !== -1) {
                        ...
                        var phantomUrl = function(url, callback) {
                            ...
                            if(url.indexOf('https://twitter.com/') === 0) {
                                var getSomeTweets = function(callback) {
                                    ...
                                    if(urlDoc.channel.meta.hasOwnProperty('screen_name')) {
                                        ...
                                        house.log.debug('getSomeTweets from @'+screenName);
                            ...
                            var youtubeUrlMatches = url.match(/(youtu\.be\/|youtube\.com\/(watch\?(.*&)?v=|(embed|v)\/))([^\?&"'>]+)/);
                            // console.log(youtubeUrlMatches)
                            if(youtubeUrlMatches && youtubeUrlMatches.length > 1) {
                                callbackErrorWithHtml("Skip phontom screenshot of youtube URL.");
                                return;
                            }
                        ...
                        var findMetaImageThumbnail = function(urlStr, html, callback) {
                            ...
                            var findUrlDocImportOgImage = function(ogImageUrl) {
                                ...
                                findMyUrlDoc(ogImageUrl, function(err, urlDoc) {
                                    if(err) {
                                        house.log.err(err);
                                    } else if(urlDoc) {
                                        if(urlDoc.ogFile) {
                        ...
                        var findFavicon = function(urlStr, html, callback) {
                            ...
                            findMyUrlDoc(faviconUrl, function(err, urlDoc) {
                                if(err) {
                                    house.log.err(err);
                                } else if(urlDoc) {
            ...
            if(docId) {
                ...
                ds.find(col, query, function(err, data) {
                    ...
                    if(doc) {
                        ...
                        removeFilesViaDoc(doc, function(){
                            house.log.debug('files removed, now remove url doc');
