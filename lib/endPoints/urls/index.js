...
(exports = module.exports = function(house, options){
    ...
    var ensureIndexes = function() {
        ...
        collection.ensureIndex({"at": -1, "url": 1}, function(){
    ...
    var cacheShareUrlHtml = function(doc, callback) {
        ...
        var phantomUrl = function(url, callback) {
            ...
            var phantomjs = spawn('phantomjs', [__dirname+'/phantomCache.js', url]);
        ...
        var procPhantom = function() {
            ...
            phantomUrl(urlsShareUrl, function(err, html, title) {
                //console.log(html);
                // to root web for now
    ...
    var findUrlDoc = function(faviconUrl, callback) {
        var query = {url: faviconUrl};
    ...
    var handleReq = function(req, res, next) {
        ...
        var countQuery = function(query, callback) {
            ...
            if(query.hasOwnProperty('q')) {
                ...
                re = re.replace(/\?/gi, '\\?');
                //{$regex: re, $options: 'gi'}
                query.url = {$regex: re, $options: 'gi'}; // var regex = new RegExp(re, "i");
                delete query.q;
            }
        ...
        var processUrls = function(urls, callback, copyOldArticles) {
            var updateUrlWithDoc = function(updateUrlDoc) {
                ds.update(col, {"_id": urlDoc.id}, updateUrlDoc, function(err, updateData) {
                    ds.find(col, {"_id": urlDoc.id}, function(err, updatedData) {
                        if(err) {
                            callback(err);
                        } else {
            ...
            var saveArticle = function(article) {
                ...
                articlesEndPoint({session: req.session, method: 'POST', url: '', fields: article}, {end:function(){}, data:function(data){
                    if(_.isArray(data)) {
                        data = _.first(data);
                    }
                    ...
                    if(beforePostDate > data.at) {
                        ...
                        ds.update(col, {"_id": urlDoc.id}, {"$push": {"articleIds": data.id}}, function(err, data) {
            ...
            house.utils.media.gridfs.importUrl(ds.db, 'files', urlDoc.url, metadata, function(err, gridStore){
                var gridfile = {
                    id: gridStore.fileId,
                    filename: gridStore.filename,
                    length: gridStore.position,
                    contentType: gridStore.contentType,
                    metadata: gridStore.metadata,
                ...
                if(err) {
                    ...
                    if(gridfile.contentType.indexOf('image') !== -1) {
                        ...
                        filesEndPoint.call(this, {session: req.session, method: 'PUT', url: '/'+gridfile.id, fields: {"$set":{"metadata.proc":0}}}, {end:function(){}, data:function(resData){
                    ...
                    } else if(gridfile.contentType == 'application/xml' || gridfile.contentType == 'text/plain') {
                        
                        var parseGridFileFeed = function(callback) {
                            //house.log.debug('parse for articles');
                            
                            var feedparser = require('feedparser');
                            mongo.GridStore.read(ds.db, gridfile.filename, gridfile.length, 0, {root: filesRoot}, function(err, data) {
                              if(err) {
                                  house.log.err(err);
                              } else {
                                //console.log(data);
                                ...
                                feedparser.parseString(data.toString(), parseOpts, callback)
                        ...
                        parseGridFileFeed(function(err, meta, articles){
                            if(err) {
                                house.log.err(err);
                            } else {
                                channel.meta = meta;
                                ...
                                if(channel.link && (!urlDoc.channel || !urlDoc.channel.link || !urlDoc.channel.url)) {
                                    ...
                                    handleReq.call(this, {session: req.session, method: 'POST', url: '', fields: metaChannelUrl}, {end:function(){}, data:function(resData){
                        ...
                        //phantom it up
                        house.log.debug('phantomjs proc');
                        var phantomUrl = function(url, callback) {
                            var html = ''
                            , title = ''
                            , desc = '';
                            ...
                            phantomjs.on('exit', function (code) {
                              ...
                              if(html.indexOf('Unable to load the address!') !== -1) {
                                  callback(new Error("Unable to load the URL."));
                              }
                        ...
                        var importOgImageUrl = function(ogImageUrl, callback) {
                            house.utils.media.gridfs.importUrl(ds.db, 'files', ogImageUrl, metadata, function(err, gridfile){
                                ...
                                if(err) {
                                    ...
                                    if(callback) {
                                        if(gridfile.contentType.indexOf('image/') === 0) {
                                            callback(ogImageUrl, gridfile);
                                        } else {
                                            callback(ogImageUrl, gridfile);
                                        }
                                    }
                        ...
                        var findMetaImageThumbnail = function(html, callback) {
                            ...
                            var findUrlDocImportOgImage = function(ogImageUrl) {
                                findUrlDoc(ogImageUrl, function(err, urlDoc){
                                    if(err) {
                                        ...
                                        if(urlDoc.ogFile) {
                                            callback(ogImageUrl, urlDoc.ogFile);
                                        }
                                    } else {
                        ...
                        var findFavicon = function(urlStr, html, callback) {
                            ...
                            // new RegExp(/<link rel="icon" href=".*?"/);
                            //var faviReg = new RegExp(/<link .*rel=".*icon.*" href=".*?"/);
                            
                            var faviReg = new RegExp(/<link.*?rel=("|\').*icon("|\').*?href=("|\')(.*?)("|\')/);
                            var faviMatches = faviReg.exec(html);
                            if(faviMatches && faviMatches.length > 4) {
                                //house.log.debug('faviMatches')
                                //house.log.debug(faviMatches)
                                faviconUrl = faviMatches[4];
                                //console.log('------------------------found favicon url, method a '+faviconUrl);
                            } else {
                                var faviRegB = new RegExp(/<link.*?href=("|\')(.*?)("|\').*?rel=("|\').*icon("|\')/);
                                var faviBMatches = faviRegB.exec(html);
                                if(faviBMatches && faviBMatches.length > 2) {
                                    //house.log.debug('faviBMatches')
                                    //house.log.debug(faviBMatches)
                                    faviconUrl = faviBMatches[2];
                                    //console.log('------------------------found favicon url, method b '+faviconUrl);
                                } else {
                                    ...
                                    var faviRegC = new RegExp(/<link.*?rel=("|\').*apple-touch-icon.*("|\').*?href=("|\')(.*?)("|\')/);
                                    var faviCMatches = faviRegC.exec(html);
                                    if(faviCMatches && faviCMatches.length > 4) {
                                        faviconUrl = faviCMatches[4];
                                    } else {
                                        var faviRegD = new RegExp(/<link.*?href=("|\')(.*?)("|\').*?rel=("|\').*apple-touch-icon.*("|\')/);
                            ...
                            var importNewFavicon = function() {
                                house.utils.media.gridfs.importUrl(ds.db, 'files', faviconUrl, metadata, function(err, gridfile){
                                    //house.log.debug('house.utils.media.gridfs.importUrl imported favicon url type:');
                                    //house.log.debug(gridfile.contentType)
                                    if(err) {
                                        house.log.debug('err');
                        ...
                        phantomUrl(urlDoc.url, function(err, phantomImagePath, html, title) {
                            if(err) {
                                ...
                                house.utils.media.fs.getFileMime(phantomImagePath, function(err, mimeType) {
                                    ...
                                    house.utils.media.gridfs.importFile(ds.db, filesRoot, 'urls/'+encodeURIComponent(urlDoc.url)+'.png', phantomImagePath, mimeType, fileMeta, function(err, gridStore){
                                        var data = {
                                            id: gridStore.fileId,
                                            filename: gridStore.filename,
                                            length: gridStore.position,
                                        ...
                                        if(err) {
                                            ...
                                            filesEndPoint.call(this, {session: req.session, method: 'PUT', url: '/'+data.id, fields: {"$set":{"metadata.proc":0}}}, {end:function(){}, data:function(resData){
                                                ...
                                                ////////////
                                                findFavicon(urlDoc.url, html, function(gridStore){
                                                    if(gridStore) {
                                                        var gridfile = {
                                                            id: gridStore.fileId,
                                                            filename: gridStore.filename,
                                                        ...
                                                        var favifile = _.clone(gridfile);
                                                        favifile.id = favifile.id;
                                                        delete favifile.id;
                                                        // update url with file
                                                        updateUrlDoc["$set"].faviconfile = favifile;
                                                        
                                                        //// Check for meta image thumb
                                                        findMetaImageThumbnail(html, function(ogImageUrl, gridStore){
                                                            if(gridStore) {
                                                                var g = {
                                                                    id: gridStore.fileId,
                                                                    filename: gridStore.filename,
                                                                ...
                                                                house.log.debug('imported og:image url to file id '+g.id);
                                                                ...
                                                                var metafile = _.clone(g);
                                                                //metafile.id = metafile.id;
                                                                //delete metafile.id;
                                                                // update url with file
                                                                //updateUrlDoc["$set"].ogImage = metafile;
                                                                //// proc image file
                                                                
                                                                // proc files of type image
                                                                if(metafile.contentType.indexOf('image') !== -1) {
                                                                    console.log('proc og/meta image file '+metafile.id);
                                                                    var filesEndPoint = house.api.getEndPointByName('files');
                                                                    filesEndPoint.call(this, {session: req.session, method: 'PUT', url: '/'+metafile.id, fields: {"$set":{"metadata.proc":0}}}, {end:function(){}, data:function(resData){
                                                                        //console.log('files response for file '+gridfile.id);
                                                                        //console.log(resData);
                                                                        if(resData.hasOwnProperty('image')) {
        ...
        */
        ...
        if(req.method == 'GET') {
            ...
            if(subPath && subPath === commentsPrefix) {
                ...
                newObject.ref = {"col": "urls", "id": docId};
            ...
            if(path == '') {
                ...
                var query = {url: newDoc.url}; //"owner.id": req.session.data.user
                console.log(query);
                var procOnPost = true;
                        ...
                        ds.find(col, query, function(err, data) {
                            ...
                            if(updateGroups) {
                                ...
                                if(updatedDoc.hasOwnProperty('groups')) {
                                    ...
                                    var setGroups = {"$set": {groups: updatedDoc.groups}};
                                    
                                    // Update images collection
                                    
                                    if(updatedDoc.hasOwnProperty('image')) {
                                        imagesEndPoint({session: req.session, method: 'PUT', url: '/'+updatedDoc.image.id, fields: setGroups}, {end:function(){}, data:function(data){
                                        },writeHead:function(){}});
                                    }
                                    if(updatedDoc.hasOwnProperty('ogImage')) {
                                        imagesEndPoint({session: req.session, method: 'PUT', url: '/'+updatedDoc.ogImage.id, fields: setGroups}, {end:function(){}, data:function(data){
                                        },writeHead:function(){}});
                                    }
                                    
                                    // Update child urls
                                    var query = {parentUrl: updatedDoc.url};
                                    ...
                                    var unsetGroups = {"$unset": {groups: true}};
                                    ...
                                    if(updatedDoc.hasOwnProperty('image')) {
                                        imagesEndPoint({session: req.session, method: 'PUT', url: '/'+updatedDoc.image.id, fields: unsetGroups}, {end:function(){}, data:function(data){
                                        },writeHead:function(){}});
                                    }
                                    if(updatedDoc.hasOwnProperty('ogImage')) {
                                        imagesEndPoint({session: req.session, method: 'PUT', url: '/'+updatedDoc.ogImage.id, fields: unsetGroups}, {end:function(){}, data:function(data){
                                        },writeHead:function(){}});
                                    }
            ...
            //if(subPath && subPath === commentsPrefix) {
                ...
                ds.remove(commentsCol, {"_id": comment_id}, function(err, data){
                    if(err) {
                        house.log.err(err);
                    } else {
