...
(exports = module.exports = function(house, options) {
    ...
    var cacheShareUrlHtml = function(doc, callback) {
        ...
        var phantomUrl = function(url, callback) {
            ...
            var phantomjs = spawn('phantomjs', ['--load-images=no', __dirname + '/phantomCache.js', url]);
        ...
        var procPhantom = function() {
            ...
            phantomUrl(urlsShareUrl, function(err, html, title, consoleOut) {
    ...
    var handleReq = function(req, res, next) {
        ...
        var emitToRoomIn = function(col, verb, doc) {
            console.log('-----------------------emitToRoomIn');
            if(!house.ioi || !house.ioi.rooms) {
            ...
            if(groups.indexOf('public') !== -1) {
                ...
                // emit to room with group permissions

                var ioiRoomManager = house.ioi.rooms. in (col).manager;
                //console.log(ioiRoomManager.handshaken.length + ' ioiRoomManager.handshaken.length')
                for(var id in ioiRoomManager.handshaken) {
                    var handshake = ioiRoomManager.handshaken[id];
                ...
                //console.log(ioRoomManager.handshaken.length + ' ioRoomManager.handshaken.length')
                ...
                if(err) {
                    ...
                    if(gridfile.contentType.indexOf('image') !== -1) {
                        ...
                        var parseGridFileFeed = function(callback) {
                            ...
                            mongo.GridStore.read(ds.db, gridfile.filename, gridfile.length, 0, {
                                root: filesRoot
                            }, function(err, data) {
                                if(err) {
                                    house.log.err(err);
                        ...
                        var importOgImageUrl = function(ogImageUrl, callback) {
                            house.utils.media.gridfs.importUrl(ds.db, 'files', ogImageUrl, metadata, function(err, ogImageUrlGridStore) {
                        ...
                        var findFavicon = function(urlStr, html, callback) {
                            ...
                            var importNewFavicon = function() {
                                house.utils.media.gridfs.importUrl(ds.db, 'files', faviconUrl, metadata, function(err, faviconGridStore) {
                                    //house.log.debug('house.utils.media.gridfs.importUrl imported favicon url type:');
                                    //house.log.debug(gridfile.contentType)
                                    if(err) {
                                        house.log.debug('err');
                                            ...
                                            filesEndPoint.call(this, {
                                                ...
                                                data: function(resData) {
                                                    ...
                                                    findFavicon(urlDoc.url, html, function(faviconGs) {
                                                        if(faviconGs) {
                                                            /*var favifile = _.clone({
                                                                id: faviconGs.fileId,
                                                                filename: faviconGs.filename,
                                                                length: faviconGs.position,
                                                                contentType: faviconGs.contentType,
                                                                metadata: faviconGs.metadata,
                                                                uploadDate: faviconGs.uploadDate
                                                            });
                                                            if (faviconGs.id) {
                                                                favifile.id = faviconGs.id;
                                                            }
                                                            if (faviconGs.length) {
                                                                favifile.id = faviconGs.length;
                                                            }*/
                                                            ...
                                                            findMetaImageThumbnail(html, function(ogImageUrl, metaiconGs, existingMetaImageUrlDoc) {
                                                                //console.log('findMetaImageThumbnail gridStore file');
                                                                //console.log(metaiconGs);
                                                                //console.log(metaiconGs.metadata.proc);
                                                                var doProc = true;
                                                                ...
                                                                if(doProc && metaiconGs) {
                                                                    /*var ogFile = _.clone({
                                                                        id: metaiconGs.fileId,
                                                                        filename: metaiconGs.filename,
                                                                        length: metaiconGs.position,
                                                                        contentType: metaiconGs.contentType,
                                                                        metadata: metaiconGs.metadata,
                                                                        uploadDate: metaiconGs.uploadDate
                                                                    });
                                                                    if (metaiconGs.id) {
                                                                        ogFile.id = metaiconGs.id;
                                                                    }
                                                                    if (metaiconGs.length) {
                                                                        ogFile.length = metaiconGs.length;
                                                                    }*/
            ...
            if(path == '') {
                ...
                findQuery(query, function(data) {
                    var respondWithFind = function(doc) {
                        console.log('url post respondWithFind');
                        var query = {
                            _id: doc.id
                        };
                        ds.find(col, query, function(err, docs) {
