...
(exports = module.exports = function(house, options){
    ...
    var handleReq = function(req, res, next) {
        ...
        var processUrls = function(urls, callback, copyOldArticles) {
            ...
            house.utils.media.gridfs.importUrl(ds.db, 'files', urlDoc.url, metadata, function(err, gridfile){
                        ...
                        phantomUrl(urlDoc.url, function(err, phantomImagePath, html, title) {
                            if(err) {
                                ...
                                house.utils.media.fs.getFileMime(phantomImagePath, function(err, mimeType) {
                                    house.utils.media.gridfs.importFile(ds.db, filesRoot, 'urls/'+encodeURIComponent(urlDoc.url)+'.png', phantomImagePath, mimeType, fileMeta, function(err, data){
                                        ...
                                        if(err) {
                                            ...
                                            filesEndPoint.call(this, {session: req.session, method: 'PUT', url: '/'+data.id, fields: {"$set":{"metadata.proc":0}}}, {end:function(){}, data:function(resData){
                                                ...
                                                ////////////
                                                findFavicon(urlDoc.url, html, function(gridfile){
                                                    if(gridfile) {
                                                        var favifile = _.clone(gridfile);
                                                        favifile.id = favifile._id;
                                                        delete favifile._id;
                                                        // update url with file
