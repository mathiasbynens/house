    ...
    var handleReq = function(req, res, next) {
        ...
        var emitToRoomIn = function(col, verb, doc) {
            if(!house.ioi || !house.ioi.rooms) {
                // house.log.debug('urls endpoint no ioi to emit to');
                return;
            }
            //console.log(doc)
        ...
        var processUrlShareCounts = function(urlDoc, callback) {
            if(urlDoc.groups && urlDoc.groups.indexOf('public') !== -1) {
                countShares.get(urlDoc.url, function(err, result){
                    if(err) {
                        house.log.err(err);
                        callback(err);
                    } else {
            ...
            house.utils.media.gridfs.importUrl(ds.db, 'files', urlDoc.url, metadata, function(err, gridStore) {
                ...
                if(err) {
                    ...
                    if(gridfile.contentType.indexOf('image') !== -1) {
                        ...
                        processUrlShareCounts(urlDoc, function(err, shareCounts){
                            if(err) {
                                house.log.debug("processUrlShareCounts err on url: "+urlDoc.url);
                                house.log.err(err);
                            }
                                    ...
                                    house.utils.media.gridfs.readFile(ds.db, filesRoot, gridfile.filename, function(err, data){
                                        if(err) {
                                            ...
                                            if(data) {
                                                ...
                                                callback(new Error("Unable to load the URL."), null, data.toString());
                                            } else {
                            ...
                            phantomUrl(urlDoc.url, function(err, phantomImagePath, html, title) {
                                if(err) {
                                    ...
                                    if(html) {
                                        ...
                                        console.log('no html from curl - just updateUrlWithDoc(updateUrlDoc)');
                                        updateUrlWithDoc(updateUrlDoc);
                                    }
