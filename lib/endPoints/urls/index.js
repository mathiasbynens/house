    ...
    var handleReq = function(req, res, next) {
        ...
        var emitToRoomIn = function(col, verb, doc) {
            ...
            if(!house.ioi || !house.ioi.rooms) {
                house.log.debug('no ioi to emit to');
                return;
            }
            //console.log(doc)
        ...
        var processUrls = function(urls, callback, copyOldArticles) {
            ...
            house.utils.media.gridfs.importUrl(ds.db, 'files', urlDoc.url, metadata, function(err, gridStore) {
                ...
                if(err) {
                    ...
                    if(gridfile.contentType.indexOf('image') !== -1) {
                    ...
                    } else if(gridfile.contentType == 'application/rss+xml' || gridfile.contentType == 'application/xml' || gridfile.contentType == 'text/xml' || gridfile.contentType == 'text/plain') {
                        var parseGridFileFeed = function(callback) {
                            ...
                            feedStream.on('end', function () {
                                house.log.debug('Done parsing feed.');
                                if(!errorState) {
                            ...
                            if(faviMatches && faviMatches.length > 4) {
                                ...
                                if(faviBMatches && faviBMatches.length > 2) {
                                    ...
                                    if(faviCMatches && faviCMatches.length > 4) {
                                        ...
                                        console.log('-found favicon url, method c '+faviconUrl);
                                    } else {
