...
(exports = module.exports = function(house, options){
    ...
    var getHandshakeFromSocket = function(socket) {
        if(!socket.id) {
            house.log.err('getHandshakeFromSocket requires socket id');
            return null;
        }
        var handshake = house.io.handshaken[socket.id];
        if(handshake && handshake.session && handshake.session.user) {
            return handshake;
        } else {
            handshake = house.ioi.handshaken[socket.id];
            if(handshake && handshake.session && handshake.session.user) {
                return handshake;
            } else {
                return null;
    ...
    var startUserStreamForSocket = function(socket) {
        var handshake = getHandshakeFromSocket(socket);
        ...
        if(handshake) {
            ...
            house.log.error(new Error("Socket handshake not found for socket.id: "+socket.id));
        }
    }
    var stopUserStreamForSocket = function(socket) {
        var handshake = getHandshakeFromSocket(socket);
        // console.log(handshake)
        if(handshake && twitterUserStreams.hasOwnProperty(socket.id)) {
            var twitterUserStream = twitterUserStreams[socket.id]; ///
            //create stream
            house.log.debug('stop twitter user stream');
            twitterUserStream.destroy();
        } else {
            house.log.err(new Error("Handshake not found for socket.id: "+socket.id));
        }
    }
