                            ...
                            if(data.contentType.indexOf('audio') === 0) {
                                        ...
                                        if(_.size(fileMeta) > 0) {
                                            updateFileId(data.id, {"$set": fileMeta}, function(err, updatedFileDoc){
                                                if(err) {
                                                    ...
                                                    house.io.emitToRoomDocAndOwnerId('files', 'inserted', updatedFileDoc, updatedFileDoc.metadata.owner.id);
                                                    callback({song: songData, file: updatedFileDoc});
                                                }
                                            });
                                        } else {
                                            house.io.emitToRoomDocAndOwnerId('files', 'inserted', data, data.metadata.owner.id);
                                ...
                                house.utils.media.exif.getFromPath(file.path, function(exif){
                                    processImage(data, exif, function(newImageData, updatedFile){
                                        house.io.emitToRoomDocAndOwnerId('images', 'inserted', newImageData, updatedFile.metadata.owner.id);
                                        house.io.emitToRoomDocAndOwnerId('files', 'inserted', updatedFile, updatedFile.metadata.owner.id);
