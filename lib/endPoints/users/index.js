    ...
    var handleReq = function(req, res, next) {
        ...
        var secureReq = function() {
            if(house.config.ssl && !req.isSecure) {
                ...
                house.log.debug('insecure connection to users endpoint, redirect to /api/users'+path);
                house.utils.response.redirect(res, 'https://' + req.headers.host + '/api/users'+path);
                return false;
            }
            return true;
        }
        ...
        var parseQuery = function(query) {
            if(query.hasOwnProperty('rewards.id')) {
                query['rewards.id'] = new ObjectID(query['rewards.id']);
            }
            return query;
        }
        ...
        if(req.method == 'GET') {
                                        ...
                                        if(err) {
                                            ...
                                            if(updatedUserData.length > 0) {
                                                ...
                                                // if(userDoc.hasOwnProperty('groups') || userDoc.hasOwnProperty('avatar')) {
                                                    ...
                                                    house.ioi.emitDocToRoomOwnerGroup(col, 'updated', userDoc, userDoc.id);
                                                    
                                                // } else {
                                                // }
                                            } else {
                                                house.log.info('user not found');
                                                res.writeHead(403);
            ...
            if(docId) {
                ...
                // TODO ability to delete your own account
