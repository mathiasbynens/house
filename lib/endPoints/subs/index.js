...
(exports = module.exports = function(house, options){
                                        ...
                                        if (err) {
                                            ...
                                            if(docs.length > 0) {
                                                ...
                                                urlsEndPoint({session: req.session, method: 'POST', url: '', fields: newUrl}, {end:function(){}, data:function(newUrlData){
                                                    ...
                                                    house.io.emitToRoomDocOwnerId(col, 'inserted', resWithDoc);
                                                },writeHead:function(){}});
                                                ...
                                                res.data(data);
                                                house.io.emitToRoomDocOwnerId(col, 'inserted', data);
                                            }
                                        }
                                    });
                                }
                        ...
                        ds.find(col, query, function(err, data) {
                            ...
                            if(putDoc.hasOwnProperty('$set') && putDoc['$set'].hasOwnProperty('url')) {
                                ...
                                urlsEndPoint({session: req.session, method: 'POST', url: '', fields: postDoc}, {end:function(){}, data:function(newUrlData){
                                    ...
                                    //res.data(resWithDoc);
                                    house.io.emitToRoomDocOwnerId(col, 'updated', updatedDoc);
                                },writeHead:function(){}});
                            }
                            var putRespond = function(data) {
                                if(insertToFeed) {
                                    insertDocToFeed(updatedDoc, function(feedDocs){
                                        ...
                                        res.data(resWithDoc);
                                        house.io.emitToRoomDocOwnerId(col, 'updated', resWithDoc);
                                    });
                                    ...
                                    data = filterData(data);
                                    res.data(data);
                                    house.io.emitToRoomDocOwnerId(col, 'updated', data);
                                }
                            }
                            if(doProc) {
                        ...
                        if(err) {
                            ...
                            ds.remove('news', {user_id: doc.owner.id, fromUrl: doc.url}, function(err, data){
                                if(err) {
                                    ...
                                    res.data({});
                                    house.io.emitToRoomDocOwnerId(col, 'deleted', docId);
                                }
                            });
                        }
                    });
	...
	var rate = house.config.subsFetcher.rate || 1000*60*60;
	if(house.config.forkedFeedFetcher) {
		house.log.debug('forked house subs feed fetcher');
		return;
	}
    var forked = child.fork(__dirname+"/fetch.js");
	var forkConfig = _.clone(house.config);
	forkConfig.forkedFeedFetcher = true;
        forked.send({"config": {config: forkConfig}});
        setInterval(function(){
            forked.send({"fetchFeeds": true}); 
        }, rate);
    }();
