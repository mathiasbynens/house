...
var fetchFeeds = function(force) {
    if(isFetching && !force) {
        house.log.debug('already fetching feeds!')
        return;
    }
    isFetching = true;
    var now = new Date();
    ...
    // TODO write a ds.stream function or .each to get them one at a time instead of the lazy toArray
                    ...
                    if(doc) {
                        ...
                        urlsEndPoint({session: sess, method: 'PUT', url: '/'+doc.id, fields: updateDoc}, {end:function(){}, data:function(newFeedData){
                            ...
                            if(data.length > 0) {
                                ...
                                ds.count(urlsCol, countquery, function(err, urlCount) {
                                    house.log.debug(urlCount+' more url channels need updating');
                                    if(urlCount > 0) {
                                        setTimeout(function(){
                                            fetchFeeds(true);
                                        }, 10000); // delay for kindness
                                    } else {
                                        isFetching = false;
                                    }
                                });
